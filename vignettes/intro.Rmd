...
People's searches on Google can tell a lot about their perception of the state of the economy and their demand for various goods and services.
...
This in turn provides an easy way to explore different keyword combinations and evaluate their econometric value and performance.


## Google Trends ##

Google Trends (https://trends.queries.google.com/) is an online search tool from Google that can be used to analyse the popularity of various Google search terms across a specified time interval.
The popularity measure of a particular search term is relative - it is a number ranging from 0 to 100 where the value of 100 marks the date when the specified search term was most popular.
...
- For a list of abailable product categories see `data("categories")`.
- For a list of available geographical regions see `data("countires)`.
- For more details about the time interval specification see `help(gtrends)`.
- For explanation of additional parameters accepted by the `ts_gtrends()` function see `help(ts_gtrends)`.
