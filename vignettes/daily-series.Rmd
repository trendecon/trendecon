...
They usually operate on the file system which means that calling these functions will write files to disk.
...
To construct an indicator, an indicator-specific script, located in the folder `inst/script`, is called.
For example, the instructions for producing the indicator for economic sentiment are stored in `inst/script/trendecon.R`.
Executing the line below will produce the data needed for this indicator:

```r
source("inst/script/trendecon.R")
```

Each such script takes a number of steps needed to prepare the indicator for final use.
...
Some keywords might display seasonal patterns.
...
Once the data for each keyword has been processed we end up with multiple time series - one for each keyword within the indicator.
...
Finally the prepared index is saved to a file:
...
- To add new indicator "indicator": create a new script at `inst/script/indicator.R`.
- To add a new geographic location "geo": create a new `proc_trendecon_geo()` function.
